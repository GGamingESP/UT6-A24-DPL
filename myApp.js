...
let arrayOfPeople = [{name: "Ivan", age: 19, favoriteFoods: ["potaje", "pizza"]}, {name: "Juan", age: 21, favoriteFoods: ["pizza"]}]
...
const removeManyPeople = (done) => {
  ...
  Person.remove({name: nameToRemove}, function (err, personFound) {
    if(err) return console.log(err);
    done(null, personFound)
  })
  // done(null /*, data*/);
...
const queryChain = (done) => {
  ...
  Person.find({favoriteFoods:foodToSearch }).sort({name: 1}).limit(2).select({name: 1, age: 0}).exec(function (err, personFound) {
    if(err) return console.log(err);
    done(null, personFound)
  })
  // done(null /*, data*/);
